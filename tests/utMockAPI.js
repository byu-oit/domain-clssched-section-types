...
describe('Mock API Unit Tests', function () {
  ...
  it("GET /{section_type} 200", () => {
    return api.request({method: 'GET', path: '/IDAHO', query: { mock: '' }})
      .then(res => { ut.testMockResponseSuccess(res, 200); });
  });
  it("PUT /{section_type}  200", () => {
    return api.request({ method: 'PUT', path: '/IDAHO', query: { mock: '' },
      body: {
        "section_type": "IDAHO",
        "description": "IDAHO",
        "long_description": "IDAHO",
  ...
  it("DELETE /{section_type} 204", () => {
    return api.request({ method: 'DELETE', path: '/IDAHO', query: { mock: '' } })
      .then(res => { ut.testMockResponseSuccess(res, 204); });
  });

  // ----- Country Code Logs -----
  it("GET /{section_type}/logs 200", () => {
    return api.request({ method: 'GET', path: '/IDAHO/logs', query: { mock: '' } })
      .then(res => { ut.testMockResponseSuccess(res, 200); });
  });
